...
/* NOTE: the goat-soup requires UNSIGNED CHAR type to work correctly */
...
char pairs0[] = "ABOUSEITILETSTONLONUTHNOALLEXEGEZACEBISOUSESARMAINDIREA."
...
char govnames[][maxlen] = {"Anarchy",      "Feudal",         "Multi-gov",
                           "Dictatorship", "Communist",      "Confederacy",
                           "Democracy",    "Corporate State"};

char econnames[][maxlen] = {"Rich Ind",     "Average Ind", "Poor Ind",
                            "Mainly Ind",   "Mainly Agri", "Rich Agri",
                            "Average Agri", "Poor Agri"};
...
char commands[nocomms][maxlen] = {"buy",  "sell",   "fuel", "jump",  "cash",
                                  "mkt",  "help",   "hold", "sneak", "local",
                                  "info", "galhyp", "quit", "rand"};

boolean (*comfuncs[nocomms])(char *) = {
    dobuy,  dosell,  dofuel,  dojump, docash,   domkt,  dohelp,
...
int myrand(void) {
  ...
  if (nativerand)
    ...
    r = (((((((((((lastrand << 3) - lastrand) << 3) + lastrand) << 1) +
              lastrand)
             << 4) -
            lastrand)
           << 1) -
...
/**+  ftoi **/
signed int ftoi(double value) { return ((signed int)floor(value + 0.5)); }

/**+  ftoi2 **/
signed int ftoi2(double value) { return ((signed int)floor(value)); }
...
void displaymarket(markettype m) {
  ...
  for (i = 0; i <= lasttrade; i++) {
    printf("\n");
    puts(commodities[i].name);
    printf("   %.1f", ((float)(m.price[i]) / 10));
    printf("   %u", m.quantity[i]);
    puts(unitnames[commodities[i].units]);
...
plansys makesystem(seedtype *s) {
  ...
  thissys.productivity =
      (((thissys.economy) ^ 7) + 3) * ((thissys.govtype) + 4);
...
void prisys(plansys plsy, boolean compressed) {
  if (compressed) {
    ...
    puts(econnames[plsy.economy]);
    printf("\nGovernment: (%i) ", plsy.govtype);
    puts(govnames[plsy.govtype]);
...
/* "Goat Soup" planetary description string code - adapted from Christian
  Pinder's reverse engineered sources. */
...
static struct desc_choice desc_list[] = {
    ...
    /* 85 */
    {{"shyness", "silliness", "mating traditions", "loathing of \x86",
      "love for \x86"}},
    /* 86 */ {{"food blenders", "tourists", "poetry", "discos", "\x8E"}},
    /* 87 */ {{"talking tree", "crab", "bat", "lobst", "\xB2"}},
    /* 88 */ {{"beset", "plagued", "ravaged", "cursed", "scourged"}},
    /* 89 */
    {{"\x96 civil war", "\x9B \x98 \x99s", "a \x9B disease", "\x96 earthquakes",
      "\x96 solar activity"}},
    /* 8A */
    {{"its \x83 \x84", "the \xB1 \x98 \x99", "its inhabitants' \x9A \x85",
      "\xA1", "its \x8D \x8E"}},
    ...
    /* 8F */
    {{"\xB0", "The planet \xB0", "The world \xB0", "This planet",
      "This world"}},
    ...
    /* 97 */
    {{"\x82 \x81 for \x8A", "\x82 \x81 for \x8A and \x8A", "\x88 by \x89",
    ...
    /* 9C */
    {{"parking meters", "dust clouds", "ice bergs", "rock formations",
      "volcanoes"}},
    ...
    /* A1 */
    {{"\x8C \x8B", "\xB1 \x9F \xA2", "its \x8D \xA0 \xA2", "\xA3 \xA4",
      "\x8C \x8B"}},
  ...
  for (;;) {
    ...
    if (c < 0x80)
      ...
      if (c <= 0xA4) {
        ...
        goat_soup(desc_list[c - 0x81].option[(rnd >= 0x33) + (rnd >= 0x66) +
                                             (rnd >= 0x99) + (rnd >= 0xCC)],
                  psy);
      } else
        switch (c) {
